...
uint8_t buffer[5];
	...
	buffer[3] = addr & 0xFF;
	buffer[4] = W25_DUMMY;// "normal/slow" read command doesn't need sending dummy byte
	Flash_Select();
...
void Flash_ReadUniqueID(uint8_t* data){
	uint8_t buffer[5];


	buffer[0] = W25_UNIQUE_ID;
	buffer[1] = W25_DUMMY;
	buffer[2] = W25_DUMMY;
	buffer[3] = W25_DUMMY;
	buffer[3] = W25_DUMMY;
...
uint8_t Flash_Init(){
	uint32_t JedecID;
	osDelay(6);	// supposing init is called on system startup: 5 ms (tPUW) required after power-up to be fully available
	Flash_Reset();
...
uint8_t command;
	...
	Flash_Transmit(&command, 1);
	Flash_UnSelect();
	osDelay(1);	// 30us needed by resetting
}
